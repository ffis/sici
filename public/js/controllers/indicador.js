(function (angular) {
	...
	angular.module('sici').controller('IndicadorCtrl', ['$scope', '$routeParams', '$rootScope', '$window', '$log', 'Indicador', 'Jerarquia', 'acumulatorFunctions', 'UsosIndicadores', 'UNIDADESINDICADOR',
		function ($scope, $routeParams, $rootScope, $window, $log, Indicador, Jerarquia, acumulatorFunctions, UsosIndicadores, UNIDADESINDICADOR) {
			...
			$scope.unidades = UNIDADESINDICADOR;
			...
			$scope.indicadores = Indicador.query({'idjerarquia': $scope.idjerarquia}, function(){
				for (let i = 0, j = $scope.indicadores.length; i < j; i += 1){
					if (typeof $scope.indicadores[i].unidad === 'undefined'){
						$scope.indicadores[i].unidad = '';
					}
			...
			UsosIndicadores.query(function(usos){
				for (let i = 0, j = usos.length; i < j; i += 1){
