  ...
  */
  ...
  function softCalculatePermiso(models, permiso) {
    ...
    if (permiso.codplaza && permiso.codplaza !== '') {
      if (restriccionProc) {
        restriccionProc.$or.push({
          'cod_plaza': permiso.codplaza
        });
      } else {
        restriccionProc = {
          'cod_plaza': permiso.codplaza
      ...
      if (restriccionEo) {
        restriccionEo.$or.push({
          'responsable': permiso.codplaza
        });
      } else {
        restriccionEo = {
          'responsable': permiso.codplaza
    ...
    attrsjerarquia.forEach(function(attr, idx) {
      ...
      jerarquiamodel.find({
        ...
        'id': true,
        'descendientes': true
      }).lean().exec().then(function(jerarquias) {
    ...
    Q.all(recalculosPendientes).then(function() {
      ...
      attrsjerarquia.forEach(function(attr, idx) {
        ...
        procedimientomodel.find({
          ...
          'codigo': true
        }, updatePermisoAttr(dP, attrProcedimiento, 'codigo', permiso));
        entidadObjetomodel.find({
          'idjerarquia': {
            '$in': idsjerarquia
          }
        }, {
          '_id': true
        }, updatePermisoAttr(dE, attrEo, '_id', permiso));
  ...
  function softCalculateProcedimientoCache(models, procedimiento, api) {
    ...
    if (idjerarquia) {
      if (typeof api === 'object') {
        ...
        jerarquiamodel.findOne({
          'id': idjerarquia
        }).lean().exec().then(function(jerarquia) {
          if (!jerarquia) {
            deferred.resolve([]);
    ...
    if (procedimiento.cod_plaza) {
      deferredPersona = personamodel.find({
        'codplaza': procedimiento.cod_plaza
      }).lean().exec();
    } else {
  ...
  function softCalculateProcedimiento(models, procedimiento) {
    ...
    if (typeof procedimiento.periodos !== 'object') {
      deferred.reject({
        error: 'Error en procedimiento ' + procedimiento.codigo
      });
  ...
  function fullSyncjerarquia(models) {
    ...
    return jerarquiamodel.find({}).exec().then(function(jerarquias) {
      ...
      }], fnActualizacion('numprocedimientos', deferNumProcedimientos, true, mapeadoArray));
      cartamodel.aggregate([{
        '$match': {
          'tipoentidad': 'CS'
        }
      }, {
      ...
      }], fnActualizacion('numcartas', deferNumCartas, false, mapeadoArray));

      //quinto esperar resultados y devolverlos
      return Q.all([deferNumProcedimientos.promise, deferNumCartas.promise]).then(function() {

        return Q.all(Object.keys(mapeadoArray).filter(function(v) {
          return typeof mapeadoArray[v] === 'object';
        }).map(function(id) {
          const d = Q.defer();

          mapeadoArray[id].save(d.makeNodeResolver());
